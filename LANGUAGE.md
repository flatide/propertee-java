...
Inside a `multi` thread, `FAIL` (and an escalating `UNWRAP`) fails only that thread — the failure is collected as `{status: "error", ok: false, value: <message>}` and the run continues, like any worker runtime error. (Stepper note: `FAIL` is an ordinary synchronous runtime error — it does not interact with the async statement-replay machinery.)
...
Implements spec v0.12.0 below: `visitFunctionDef` rejects all-uppercase function names at definition time. **Breaking** for scripts that define such functions (the error pinpoints the definition line). Language semantics only.
...
**Migration note (breaking):** rename any script-defined all-uppercase function — the error pinpoints the definition line. The shadow-mocking pattern (`function SLEEP(...)` to stub a built-in) is no longer possible — inject a host external or use a lowercase wrapper instead. Variable names are unaffected.
...
Implements the spec v0.10.0 additive batch below — `FAIL`/`UNWRAP`/`OK`/`ERR`/`IS_RESULT` and the genuine-Result origin (`runtime.TeeResult`). **Non-breaking** for scripts and hosts; language semantics only.
