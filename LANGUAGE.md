...
**Non-breaking**: no grammar or keyword changes; no existing behavior changes; existing fixtures are byte-identical. Scripts (or hosts) that define/register functions under these five names shadow the new built-ins and keep their existing behavior. `UNWRAP` accepts only genuine Results — replace hand-built `{"status": ..., "ok": ..., "value": ...}` literals with `OK(v)`/`ERR(v)` where they must interoperate with `UNWRAP`.
