...
Source layout: `propertee-{module}/src/main/java/com/flatide/propertee/{package}/`. Grammar at `propertee-core/grammar/ProperTee.g4`. Tests at `propertee-core/src/test/`.
...
# Run the whole fixture suite (Gradle 9's --tests cannot match parameterized display names,
# so single-fixture selection like testScript[09_functions] is not possible)
./gradlew :propertee-core:test --tests "com.flatide.propertee.tests.ScriptTest"
...
**Task coverage:** the SHELL task fixtures (`72_shell`, `78_task_basic`, `80_task_unique_ids`) run through ScriptTest and spawn real shell processes; `UnsupportedTaskRunnerTest` covers the no-runner error surface. (The former `TaskEngineTest` was deleted when the platform task runners left core; multi-instance and archival tests live in TeeBox's ManagedTaskEngineTest.)
