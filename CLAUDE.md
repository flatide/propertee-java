...
**Remaining eager seam — blocking fallback.** A suspendable call **embedded in an expression** is still evaluated eagerly: assignment RHS (`x = foo()`), operators (`a + foo()`), `if`/`loop` conditions, loop iterables, and function arguments. A `SLEEP` reached that way (via `evalBlock`/`callUserFunction`) is honored with a **blocking `Thread.sleep` fallback** — correct timing, but it blocks the scheduler thread for that sleep. **Monitor bodies** also run synchronously by design (via `evalBlock`) and keep the blocking fallback. Single-threaded scripts are unaffected. Closing this seam requires expression-level stepping (Strategy A/B), tracked separately. Regression coverage: `SleepNestingTest` (timing) and `CooperativeNestingTest` (cooperative interleaving: overlap ~1× vs serialized ~2×).
