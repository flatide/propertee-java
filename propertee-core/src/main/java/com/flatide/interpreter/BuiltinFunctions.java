...
import com.flatide.runtime.AsyncPendingException;
...
import com.flatide.platform.UnsupportedPlatformProvider;
...
public class BuiltinFunctions {
    ...
    private void registerDefaults() {
        ...
        registerResult("SHELL_CTX", new BuiltinFunction() {
            ...
            public Object call(List<Object> args) {
                if (args.isEmpty()) {
                    return com.flatide.runtime.Result.error("SHELL_CTX() requires at least 1 argument (cwd)");
                }
                Object cwdArg = args.get(0);
                if (!(cwdArg instanceof String)) {
                    return com.flatide.runtime.Result.error("SHELL_CTX() first argument must be a string (directory path)");
                }
                String cwd = (String) cwdArg;
                File dir = new File(cwd);
                if (!dir.exists() || !dir.isDirectory()) {
                    return com.flatide.runtime.Result.error("Directory does not exist: " + cwd);
                }
                ...
                if (args.size() >= 2) {
                    ...
                    if (!(envArg instanceof Map)) {
                        return com.flatide.runtime.Result.error("SHELL_CTX() second argument must be an object (environment variables)");
                    }
                ...
                if (args.size() >= 3) {
                    ...
                    if (!TypeChecker.isNumber(timeoutArg)) {
                        return com.flatide.runtime.Result.error("SHELL_CTX() third argument must be a number (timeout ms)");
                    }
                    ctx.put("timeout", timeoutArg);
                }
                return com.flatide.runtime.Result.ok(ctx);
            }
        });
        ...
        functions.put(name, new BuiltinFunction() {
            ...
            public Object call(java.util.List<Object> args) {
                try {
                    ...
                    return com.flatide.runtime.Result.error(e.getMessage());
                }
            }
        });
    }
